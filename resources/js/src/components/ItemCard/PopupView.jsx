...
export default function PopupView({ onClose, image, name, lang, size, discount, ingredients }) {
    return (
        <>
                ...
                <div className="item-info">
                                    ...
                                    Array.isArray(ingredients) ? (
                                        <ul>
                                            {ingredients.map((i, index) => (
                                                <li key={index + '_ingredients'}>{i.name[lang]}</li>
                                            ))}
                                        </ul>
                                    ) : null
                                }
                    ...
                    </div>
                    {
                        (size.discount_price ? size.discount_price : size.price) > 0 ? (
                            <span>
                                {size.discount_price ? size.discount_price : size.price}{Constants.currency}
                            </span>
                        ) : <span/>
                    }
                </div>
